    ...
    <div className="space-y-8">
      <div className={`rounded-[3rem] p-8 relative transition-all duration-700 ${isLocked ? 'glass-panel-heavy overflow-hidden' : `glass-panel-heavy border-2 ${styles.border} shadow-2xl`}`}>
        ...
        <div className={`flex flex-col gap-6 ${isLocked ? 'opacity-20 grayscale' : ''}`}>
          <div 
            onClick={() => !isLocked && !image && fileInputRef.current?.click()} 
          ...
          >
            {image ? (
              <div className="relative w-full h-full group">
                <img 
                  src={image} 
                  className={`h-full w-full object-contain p-4 transition-transform duration-500 ${isZoomed ? 'scale-150 cursor-zoom-out' : 'cursor-zoom-in'}`} 
                  alt="Chart" 
                ...
                <div className="absolute bottom-4 left-4 flex gap-2">
                  <button 
                    ...
                    className="p-2 rounded-xl bg-black/60 text-white hover:bg-rose-600 transition-all border border-white/10"
                    title="حذف الصورة"
                  >
                  ...
                  <button 
                    onClick={() => setIsZoomed(!isZoomed)} 
                    className="p-2 rounded-xl bg-black/60 text-white hover:bg-indigo-600 transition-all border border-white/10"
                    title="تكبير/تصغير"
                  >
